...
**현재 상태: 설계 단계, 코드 0줄.** 작업계획서는 `docs/plan.html`. 구현 전 반드시 그 문서를 먼저 읽을 것.
...
4. **번인(burn-in)은 필수, 단 다운스케일 후 최종 해상도에서 렌더링.** 번호 배지는 대상 픽셀을 가리지 않게 8~12px 오프셋 + 리더 라인. 노트 본문 텍스트는 번인하지 않는다(텍스트 채널 담당).
