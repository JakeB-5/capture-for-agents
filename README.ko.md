...
이름은 "고칠 곳이 **여기요!!**" 하며 화면을 손가락으로 콕 가리키는 그 느낌입니다. (영문 표기: *Yeogiyo!!* · 구명: Capture for Agents)

> **상태: ✅ Phase 3 완료 (2026-07-03) — 배포 가능 빌드.** 캡처 → 어노테이션 → ⌘V 전 루프 실기기 검증, 히스토리·설정·릴리스 DMG(ad-hoc 서명)까지 완료. 공증(계정)만 보류 — 멀티모니터 좌표는 외부 4K 모니터 실측 통과. 상세 작업계획서는 [docs/plan.html](docs/plan.html)에 있습니다.
...
여기요!!의 해법은 **번호 라벨 + 부연설명 + 이중 채널**입니다.
...
| 3 | ✅ **완료 (2026-07-03)** 폴리싱·견고화 — 창 표시 속도, 키보드-온리, 히스토리 10건(재복사/재어노테이션), 배지 충돌 회피, 싱글 인스턴스, 설정(단축키/경로/보존), 터미널 3종 실측, 4K 멀티모니터 좌표 실측, 지목 정확도 평가, 릴리스 DMG(ad-hoc). 공증·멀티모니터 실측 보류. [검증 리포트](docs/phase3-verification.md) | 2~3d |
...
[MIT](LICENSE) · 기여 워크플로와 동결 스펙 규칙은 [CONTRIBUTING.md](CONTRIBUTING.md)(영문)를 참고하세요.
